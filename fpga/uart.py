...
class UART(Elaboratable):
    """
    This module is taken from `https://github.com/m-labs/nmigen/blob/master/examples/basic/uart.py`
    ...
    divisor : int
        ...
        E.g. ``12e6 / 115200`` = ``104``.
    """
    def __init__(self, clk_freq: float, baud_rate: float, data_bits:int=8):
        divisor = round(float(clk_freq) / float(baud_rate))
        assert divisor >= 4
